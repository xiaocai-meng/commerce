...
class Order extends ActiveRecord
{
    ...
    public function rules() {
        return [
            [['userid', 'status'], 'required', 'message' => 'userid和status是必须的', 'on' => 'add'],
    ...
    public static function getData($order) {
        $details = OrderDetail::find()->where('orderid = :orderid', [':orderid' => $order->orderid])->all();
        $products = [];
        foreach ($details as $detail) {
            $product = Product::find()->where('id = :id', [':id' => $detail->productid])->one();
            if (empty($product)) {
                continue;
            }
        ...
        $user = User::find()->where('userid = :userid', [':userid' => $order->userid])->one();
        if (!empty($user)) {
            $order->username = $user->username;
        }
        $order->address = Address::find()->where('addressid = :addressid', [':addressid' => $order->addressid])->one();
    ...
    public static function getProducts($userid) {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($details as $detail) {
                ...
                $product->cate = Category::find()->where('id = :id', [':id' => $product->cateid])->one()->title;
                $products[] = $product;
            }
            $order->zhstatus = self::$status[$order->status];
            $order->products = $products;
